...
class SpriteStyleNode{
  ...
  public function renderBackground(array $params = array()){
    if($this->backgroundNode){
      ...
      return 'background: url(\''.SpriteConfig::get('cssImagePath').$this->background_image.'\') '.$background.' ; ';
    }
  }
  ...
  public function renderPadding(array $params = array()){
    return (SpriteConfig::get('forceNoPadding'))?('padding: 0 !important;'):('');
  }
  public function renderSupplements(array $params = array()){
    return $this->renderPadding($params);
  }
  ...
  public function renderStyle(array $params=array()){
      ...
      if(!$this->backgroundNode){
        ...
        if(@$params['inline']){
          return $this->renderSupplements($params).$this->renderBackground($params).$this->renderBackgroundPosition($params).$this->renderSize();
        }
        else{
          return $this->renderSupplements($params).$this->renderBackgroundPosition($params).$this->renderSize();
        }
      }
  }
