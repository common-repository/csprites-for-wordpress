...
class SpriteImageWriter{
  ...
  public static function writeImages(SpriteSprite &$sprite){
    ...
    if(SpriteCache::needsCreation(SpriteConfig::get('rootDir').$filePath)){
  ...
  protected static function handleJpg(SpriteSprite $sprite, $imgSize, $filePath){
    ...
    foreach($sprite as $spriteImage){
      ...
      if($spriteImage->needsResize()){
        $newSize = $spriteImage->getNewSize();
        $sourceRectangle = $newSize->getResizedRectangle($spriteImage->getOriginalRectangle(), $spriteImage->getCrop()); 
        imagecopyresampled($spriteHolder, $tempImage, $position->left + $margin->left, $position->top + $margin->top, $sourceRectangle->left, $sourceRectangle->top, $newSize->width , $newSize->height, $sourceRectangle->width, $sourceRectangle->height);
      }
      else{
  ...
  protected static function handleGif(SpriteSprite $sprite, $imgSize, $filePath){
    ...
    foreach($sprite as $spriteImage){
      ...
      if($spriteImage->needsResize()){
        ...
        imagecopyresized($spriteHolder, $tempImage, $position->left + $margin->left, $position->top + $margin->top, $sourceRectangle->left, $sourceRectangle->top, $newSize->width , $newSize->height, $sourceRectangle->width, $sourceRectangle->height);
  ...
  protected static function handlePng24(SpriteSprite $sprite, $imgSize, $filePath){
    $properties = SpriteConfig::get('imageProperties');
    $quality = $properties['pngCompression'];
    ...
    ob_start();
    imagepng($spriteHolder, null, $quality, PNG_ALL_FILTERS);
    $spriteOutput = ob_get_clean();
    imagedestroy($spriteHolder);
    SpriteImageWriter::writeImageFile($spriteOutput, $filePath);
  }
